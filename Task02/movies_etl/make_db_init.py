...
def generate_ratings(writer):
    ...
    writer.write(
        "CREATE TABLE ratings (id INTEGER PRIMARY KEY, user_id INTEGER, movie_id INTEGER, rating REAL, timestamp TEXT);\n")
...
def generate_tags(writer):
    ...
    writer.write(
        "CREATE TABLE tags (id INTEGER PRIMARY KEY, user_id INTEGER, movie_id INTEGER, tag TEXT, timestamp TEXT);\n")
...
def generate_users(writer):
    ...
    with open(FILES["users"], encoding="utf-8") as f:
        for i, line in enumerate(f, 1):
            ...
            rows.append(
                f"({u_id}, '{escaped_name}', '{escaped_email}', '{gender}', '{reg_date}', '{escaped_occupation}')")

            if len(rows) >= BATCH_SIZE:
                write_batch_insert(writer, "users", ["id", "name", "email", "gender", "register_date", "occupation"],
                                   rows)
                rows = []
